...
- Reading `filt` instead of the raw oscillation costs waiting time, and the procedure says so: it is an EMA with τ ≈ 100 s, so 60 s after a 10 L pour some 55 % of the step is still unresolved. The documented wait per step is ~5 min, making the full 11-point calibration close to an hour. Recording too early biases every row the same way, which the table's monotonicity `static_assert` cannot catch — the curve stays plausible and is permanently wrong.
